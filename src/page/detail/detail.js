  ...
  $.ajax({
    ...
    success: function(res) {
      ...
      if (res.code == "200") {
        ...
        var oneConternHtml =
          '<div class="ShareDetailsBlock ShareDetailsBlockAnother"><div class="left"><a><img style="width: 347px;height: 260px;" src="' +
          StaticUrl +
          fenleidata.url +
          '">' +
        ...
        if (_idsType == 10) {
          _attrI = 4;
          var _titleBody =
            '<div class="ShareDetailsBlock ShareDetailsBlockAnother"><div class="left"><a>' +
            '<img style="width: 347px;height: 260px;" src="' +
            StaticUrl +
            res.data.url +
            '"></a>' +
            '</div><div class="right"><h1><a>' +
            res.data.title +
            "</a></h1>" +
            "<dl><dt>别名</dt><dd>" +
            res.data.attrs[0].attrValue +
            "</dd></dl><dl><dt>英文名称</dt><dd>" +
            res.data.attrs[1].attrValue +
            "</dd></dl><dl>" +
            "<dt>其他英文名</dt><dd>" +
            res.data.attrs[2].attrValue +
            "</dd></dl><dl><dt>CAS</dt><dd>" +
            res.data.attrs[3].attrValue +
            "</dd></dl></div></div>";
        } else {
          var _titleImg = ""; //工具的 还有图片
          if (res.data.url) {
            _titleImg =
              '<div class="Video"><img style="height: 228px;" src="' +
              StaticUrl +
              res.data.url +
              '" /></div>'; //工具的 还有图片
          }
          var _titleBody =
            '<div class="ShareDetailsBlock MediaBlockAnother"><div class="title">' +
            res.data.title +
            "</div>" +
            _titleImg +
            "</div>";
        ...
        for (var attrI = _attrI; attrI < otherAttrs.length; attrI++) {
          ...
          if (
            ...
            attrValueNull = "";
            ulNavListHtml +=
              '<li disabled="disabled" class="li-disabled" name="level' +
              attrI +
              '" level=' +
              attrI +
              '><a style="color:#909090;"><span>' +
              otherAttrs[attrI].prop.attrName +
              "</span></a></li>";
          } else {
            tableHtml +=
              '<div class="ShareDetailsBlock ShareDetailsBlockOther" id="level' +
              attrI +
              '"><div class="title">' +
              otherAttrs[attrI].prop.attrName +
              "</div>" +
...
function getOtherData() {
  ...
  $.ajax({
    type: "post",
    url:
      "http://safety.kahntang.com/search/getReltRepo?token=" +
      token +
      "&entityId=" +
      repoId,
    data: {},
        ...
        for (var oi = 0; oi < otherShareData.length; oi++) {
          ...
          if (oi == 0) {
            for (var ii = 0; ii < otherShareData[oi].data.length; ii++) {
              ...
              _otherSmallData +=
                '<div class="ClassicCaseCon"><a href="/detail.html?repoId=' +
                otherShareData[oi].data[ii].id +
                '" target="_blank">' +
                otherShareData[oi].data[ii].title +
                "</a><p>" +
                _iscontNull +
                "</p></div>";
            }
            otherShareHtml +=
              '<div class="ClassicCase" style=""><div class="big-title"><span>相关知识图谱</span></div><div class="title">' +
              otherShareData[oi].data[0].menu.menuName +
              '</div><div class="ClassicCaseBlock">' +
              _otherSmallData +
              "</div></div>";
          } else {
            for (var ii = 0; ii < otherShareData[oi].data.length; ii++) {
            ...
            otherShareHtml +=
              '<div class="ClassicCase" style=""><div class="title">' +
              otherShareData[oi].data[0].menu.menuName +
              '</div><div class="ClassicCaseBlock">' +
              _otherSmallData +
              "</div></div>";
...
$(function() {
  ...
  $("#elevator").on("click", "li", function() {
    ...
    $("#elevator")
      .find(".ui-elevator-select")
      .removeClass("ui-elevator-select");
  ...
  $(window).scroll(function() {
    ...
    if (scrollTops >= startTop && scrollTops <= endTop) {
      //当前刷新时
      $("#elevator")
        .find(".ui-elevator-select")
        .removeClass("ui-elevator-select");
      ...
      if (level + 1 < levelShows.length) {
        //下滑时
        level = level + 1;
        $("#elevator")
          .find(".ui-elevator-select")
          .removeClass("ui-elevator-select");
        $("[name=level" + levelShows[level] + "]").addClass(
          "ui-elevator-select"
        );
      } else {
        $("[name=level" + levelShows[level] + "]").removeClass(
          "ui-elevator-select"
        );
      }
    } else if (scrollTops < startTop) {
      //上滑时
      if (level > 0) {
        level = level - 1;
        $("#elevator")
