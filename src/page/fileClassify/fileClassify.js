    ...
    success: function(res) {
      ...
      if (res.code == "200") {
        ...
        for (var j = 0; j < contentResult.length; j++) {
          RightContentHtml +=
            '<div class="ToolUse_Right_List" style="padding:12px 0;">' +
            '<h1 class="gotoDetail" repoId=' +
            contentResult[j].id +
            '><a style="line-height: 18px;font-weight:normal;font-size: 18px;background: url(http://safety.yooso.com.cn/resources/images/title_icon.png) no-repeat left center;padding-left: 20px;" target="_blank" detailid="' +
            contentResult[j].id +
            '" href="' +
            StaticUrl +
            contentResult[j].content +
            '"' +
            ">" +
            contentResult[j].title +
            "</a></h1>" +
            "</div>";
        }
          ...
          for (var i = 0; i < result.length; i++) {
            if (result[i].dictItemId == null || result[i].dictItemId == 0) {
              ...
              for (var ulJ = 0; ulJ < result[i].childs.length; ulJ++) {
                ul_MenusHtml +=
                  '<li><a class="erjimenuli" menuid=' +
                  result[i].childs[ulJ].id +
                  " title=" +
                  result[i].childs[ulJ].menuName +
                  "id=" +
                  result[i].childs[ulJ].id +
                  ">" +
                  result[i].childs[ulJ].menuName +
              ...
              LeftMenuHtml +=
                '<li class="btn_showUl" id="menu' +
                result[i].id +
                '" menuid=' +
                result[i].id +
                ">" +
                '<a class="u_title_btn" title=' +
                result[i].menuName +
                "id=" +
                result[i].id +
                ">" +
                ...
                '<ul id="ulList' +
                result[i].id +
                '" class="TabTwo" style="display:none;">' +
                ul_MenusHtml +
                "</ul></li>";
            }
          }
        ...
        if (res.data.values.length > 0) {
          $(".ToolUse_Title").html(
            res.data.values[0].menu.menuName +
              "（" +
...
$(document).scroll(function() {
  ...
  if (scrollTop > min && winHe > leftHe && leftHe < rightHe) {
    $(".ToolUse_Left").attr(
      "style",
      "position: fixed;z-index: 1000;top: 0px;  margin-top: 0px;"
    );
  } else {
...
function getSearchData(page) {
  ...
  $.ajax({
    ...
    url:
      ...
      "&keyword=" +
      searchText +
      "&style=SUMMARY&highlight=true&labelId" +
      _labelId,
    data: {},
    dataType: "json",
    success: function(res) {
