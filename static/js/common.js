...
$("#back_getVerCode").bind("click", function() {
  ...
  if (countdown == 120) {
    ...
    var _sign = md5(
      "appId=10001&mobile=" + _telPhone + "&stime=" + _stime + _miyao
    );
    $.ajax({
      type: "post",
      url:
  ...
  $.ajax({
    ...
    success: function(res) {
      ...
      if (res.code == "200") {
        ...
        LoginUl +=
          '<a href="/login/UserInfo">欢迎您：<span>' + account + "</span></a>";
        LoginUl += "</li>";
...
function outEdit() {
  ...
  $.ajax({
    ...
    success: function(data) {
      ...
      localStorage.setItem("phoneNumber", ""); //缓存手机号
...
var _alert =
  '<div class="alert alert-warning alert-positon"><a href="#" class="close" data-dismiss="alert">&times;</a><strong id="aleertText"></strong></div>';
$("body").append(_alert);
